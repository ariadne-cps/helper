...
#ifndef ARIADNE_UTILITY_UNIFORM_ARRAY_HPP
#define ARIADNE_UTILITY_UNIFORM_ARRAY_HPP
    ...
    : private PR
{
  private:
    using _pr=PR;
    static T* uninitialized_new(std::size_t n) { return static_cast<T*>(::operator new(n*sizeof(T))); }
  ...
  private:
    ...
    template<class G> requires InvocableReturning<ValueType,G,std::size_t>
    static CharacteristicsType _get_generator_characteristics(std::size_t n, G const& g) {
    ...
    template <typename F, typename TUP> static decltype(auto) call(F f, TUP && t) {
        constexpr std::size_t N = std::tuple_size<std::decay_t<TUP>>::value; return call_impl<F, TUP, N==0, N>::call(f, std::forward<TUP>(t)); }
  ...
  public:
    ...
    explicit UniformArray(const std::size_t n) : UniformArray(n,_default_characteristics()) { }
    //! \brief Constructs a UniformArray of size \a n with default-initialised elements.
    explicit UniformArray(const std::size_t n, CharacteristicsType pr) : PR(pr), _size(n), _ptr(uninitialized_new(n)) { for(std::size_t i=0; i!=n; ++i) { new (_ptr+i) T(_make_default()); } }
    ...
    explicit UniformArray(const std::size_t n, CharacteristicsType pr, Uninitialised) : PR(pr), _size(n), _ptr(uninitialized_new(n)) { }
    //! \brief Constructs a UniformArray of size \a n with elements initialised to \a x.
    UniformArray(const std::size_t n, const ValueType& x) : PR(get_characteristics(x)), _size(n), _ptr(uninitialized_new(n)) { this->_uninitialized_fill(x); }
    ...
    template<class G> requires InvocableReturning<ValueType,G,std::size_t>
    UniformArray(std::size_t n, G const& g)
    ...
    UniformArray(ForwardIterator first, ForwardIterator last)
            : _pr(_get_iterator_range_characteristics(first,last)), _size(static_cast<std::size_t>(std::distance(first,last))), _ptr(uninitialized_new(_size)) {
    ...
    template<class G, class... PRS> requires InvocableReturning<ValueType,G,std::size_t> && Constructible<ValueType,PRS...>
    UniformArray(std::size_t n, G const& g, PRS... prs)
    ...
    template<class G, class... PRS> requires InvocableReturning<ValueType,G,std::size_t> && Constructible<ValueType,PRS...>
    UniformArray(std::size_t n, G const& g, Tuple<PRS...> prs)
    ...
    std::size_t max_size() const { return (std::size_t) (-1); }
    ...
    void resize(std::size_t n) {
        if(size()!=n) {
            ...
            for(std::size_t i=0; i!=n; ++i) { if(i<_size) { new (_new_ptr+i) T(_ptr[i]); } else { new (_new_ptr+i) T(_make_default()); } }
    ...
    void resize(std::size_t n, const T& t) {
        if(size()!=n) {
            pointer _new_ptr=uninitialized_new(n);
            for(std::size_t i=0; i!=n; ++i) { if(i<_size) { new (_new_ptr+i) T(_ptr[i]); } else { new (_new_ptr+i) T(t); } }
            this->_destroy_elements(); uninitialized_delete(_ptr); _size=n; _ptr=_new_ptr; } }
    //! \brief Reallocates the UniformArray to hold \a n elements. The new elements are zero-constructed.
    void reallocate(std::size_t n) { if(size()!=n) { this->_destroy_elements(); uninitialized_delete(_ptr);
        _size=n; _ptr=uninitialized_new(_size); for(std::size_t i=0; i!=_size; ++i) { new (_ptr+i) T(_make_default()); } } }
    ...
    const ValueType& get(std::size_t i) const { return _ptr[i]; }
    //! \brief Get the \a n th element (unchecked).
    void set(std::size_t i, const ValueType& x) const {
    ...
    ElementReference<ValueType> operator[](std::size_t i) { return static_cast<ElementReference<ValueType>>(_ptr[i]); }
    //! \brief The \a n th element.
    const ValueType& operator[](std::size_t i) const { return _ptr[i]; }
    //! \brief Checked access to the \a n th element.
    ElementReference<ValueType> at(std::size_t i) {
    ...
    //! \brief Checked access to the \a n th element.
    const ValueType& at(std::size_t i) const { if(i<_size) { return _ptr[i]; } else { throw std::out_of_range("UniformArray: index out-of-range"); } }
...
template<class T, class PR> std::ostream& operator<<(std::ostream& os, const UniformArray<T,PR>& a) {
